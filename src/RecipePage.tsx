        ...
        {recipe && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {!/^S/i.test(item.code) && item.code !== 'VSM' && item.code !== 'CRCP' && <StatCard icon={<Thermometer className="w-5 h-5 text-emerald-500" />} value={`${recipe.cookTempF || '--'}°F`} label="Cook Temp" />}
              {!/^S/i.test(item.code) && item.code !== 'VSM' && item.code !== 'CRCP' && <StatCard icon={<Thermometer className="w-5 h-5 text-amber-500" />} value={`${recipe.holdTempF || '--'}°F`} label="Hold Temp" />}
